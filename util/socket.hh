...
/**
 * @brief Socket封装类
 * 原来的创建用法socket(AF_INET, SOCK_STREAM, 0);
 */
...
public:
    ...
    /**
     * @brief 接收connect链接（connect是客户端进行操作的，服务端是accept接收的）
    ...
    /**
     * 自己增加的函数，因为http_conn类要用到
     */
    virtual ssize_t writev(const struct iovec *iov, int iovcnt); //自己增加的函数
...
protected:
    ...
    int m_sock;
    /// 协议簇
    int m_family; //表示是ipv4这种还是v6这种的
    /// 类型
    int m_type; //tcp 或者udp
    /// 协议
    int m_protocol; //一般选择了m_type就不用管再选了，是socket的第三个参数，设置为0系统会自己帮你选择
    /// 是否连接
    bool m_isConnected;
    /// 本地地址
